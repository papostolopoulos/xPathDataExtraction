function transform(data){
  ...
  var negRegexArr = [
    /^\s*Get\s*coupon\s*Print\s*or\s*Send\s*to\s*Card\s*through.*/i,
    /^Save\s*on\s*what\s*you\s*love,\s*what\s*you\s*need\s*and\s*what\s*you've\s*always\s*wanted\s*to\s*try\.?/i,
    /^Your\s*\$\d+\s*in\s*ExtraBucks\s*Rewards\s*expire\s*on\s*(\d{1,2}\/{2})\d{2,4}/i,
    /^Your\s*\d{1,2}%\s*discount\s*on\s*CVS\s*Health\s*brand\s*products\s*has\s*also\s*been\s*loaded\s*to\s*your\s*ExtraCare\s*card/i
  ];
...
function cleanMe(string) {
  ...
  var replaceStrArr = [
    {oldStr: /^(\s*\$\d+(?:\.\d{2})?.*ExtraBucks\s*Rewards)(.*)/, newStr: "$2: $1"},
    {oldStr: /^(—|Plus,?|Rewards)\s*/, newStr: ""},
    {oldStr: /(Send\s*to\s*my\s*card|Redeem\s*through).*/, newStr: ""},
    {oldStr: /(Get\s*coupon|in\s*store\s*or\s*online|Shop\s*eligible\s*products)\s*$/i, newStr: ""},
    {oldStr: /Rewards Rewards/, newStr: "Rewards"}
  ];

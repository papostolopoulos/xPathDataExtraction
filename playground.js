function transform(data){
  ...
  if (data.search(/FREE|Today\s*Only/i) !== -1 && data.length <= 10) return null;
  ...
  var posRegexArr = [
    ...
    //promo(tion) code
    /promo(?:tion)\s*code/i,
    /free|today\s+only|final\s+hours|only!|last\s+chance/i,
  ];
...
function cleanMe(string) {
  ...
  var replaceStrArr = [
    ...
    {oldStr: /((through|on)\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},\s+\d{4}).*/i, newStr: ""}, // Find text in the middle, remove everything after that.*
